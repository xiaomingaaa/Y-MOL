...
def main():
    ...
    base_path = '/zengdaojian/litianle/Mycode/Mol-chat/LLaMA-Factory/metric/Property'
    mae = []
    error = []
    file_names = read_jsonl_lists(base_path)
    for file_path in file_names:
        # 读取JSON Lines数据
        data = read_jsonl_file(file_path)
        # 提取标签和预测值，并转换为float类型
        lines = [[float(item['label'][:-1]), float(item['predict'])] for item in data if item['predict'] != '']
        
        labels = [item[0] for item in lines]
        predictions = [item[1] for item in lines]
        # 计算MAE
        mae_value = calculate_mae(labels, predictions)
        print(f"{file_path.split('/')[-1].split('.')[0]} MAE: {mae_value} Error: {len(data) - len(lines)}")
        mae.append(mae_value)
        error.append(len(data) - len(lines))
    # 要写入的CSV文件路径
    csv_file_path = '/zengdaojian/litianle/Mycode/Mol-chat/LLaMA-Factory/metric/mae_llama2_results.csv'
    # 打开文件，写入模式
    with open(csv_file_path, mode='w', newline='') as file:
        writer = csv.writer(file)
        # 写入标题行（如果有的话）
        writer.writerow(['Name', 'MAE', 'Error'])
        # 写入数据行
        for name, mae, e in zip(file_names, mae, error):
            writer.writerow([name.split('/')[-1].split('.')[0], mae, e])
