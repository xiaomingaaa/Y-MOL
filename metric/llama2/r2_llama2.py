...
def main():
    ...
    for file_path in file_names:
        ...
        print(f"{file_path.split('/')[-1].split('.')[0]} R^2: {r2_value} Error: {len(data) - len(lines)}")
        r2.append(r2_value)
        error.append(len(data) - len(lines))
    # 要写入的CSV文件路径
    csv_file_path = '/zengdaojian/litianle/Mycode/Mol-chat/LLaMA-Factory/metric/r2_llama2_results.csv'
    # 打开文件，写入模式
    with open(csv_file_path, mode='w', newline='') as file:
        writer = csv.writer(file)
        # 写入标题行（如果有的话）
        writer.writerow(['Name', 'R^2', 'Error'])
        # 写入数据行
        for name, mae, e in zip(file_names, r2, error):
