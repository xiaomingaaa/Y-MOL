...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_folder",
        type=str,
        default='/zengdaojian/litianle/Mycode/Mol-chat/LLaMA-Factory/metric/Eval_smiles_llama2/data'
    )
    ...
    for name in files_name:
        # 写入的文件
        f = open('/zengdaojian/litianle/Mycode/Mol-chat/LLaMA-Factory/metric/Eval_smiles_llama2/save/' + name.split('/')[-1].split('.')[0] + '.jsonl', 'w', encoding='utf-8')
        datas = read_json_data(name)
        for data in tqdm(datas):
            # data = datas[i]
            result = {}
            inputs =  data['instruction'] + ' Please give the molecule generation as required.'
            ...
            for seq in sequences:
                output = seq['generated_text'].split('Please give the molecule generation as required.')[1]
            result['label'] = data['instruction']
